...
to execute-actions
  ...
  ask turtle 0 [
    ask patches in-radius 1 [
      if(not turning) [
        set turning pcolor != black and pcolor != yellow
      ]
    ]
  ]

  if(turning and not goodtogo) [random_direction stop]

  if(random 100 <= 20) [random_direction stop]

  if(not goodtogo) [do "forward"]
...
to-report facing [n]
  ...
  if (n = "west" and [heading] of turtle 0 = 270) [report true]
  if (n = "south" and [heading] of turtle 0 = 180) [report true]
  report false
end
...
; Procedure to report if the patch ahead is dirty or unvisited
to-report goodtogo
  let ans true
  ask turtle 0 [
    if (patch-ahead 1 != nobody)[
      set ans ([pcolor] of patch-ahead 1 = grey or [pcolor] of patch-ahead 1 = white)
    ]
  ]
  report ans
end
