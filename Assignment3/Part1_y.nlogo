...
to update-desires
  ...
  if (count patches with [pcolor = grey] > 0) [
    ask vacuums [set desire "clean-all"] stop
  ]


  ask vacuums [set desire "time-to-stop"] stop
end
...
to update-intentions
  ...
  ask vacuums [
    if (desire = "clean-all" and [pcolor] of patch-here = grey ) [
      set intention "suck-now" stop
    ]
    if (desire = "clean-all" and not head-to-close) [
      set intention "head-closest" stop
    ]
    if (desire = "clean-all") [
      set intention "move" stop
    ]
  show "done"
  ]
end
